...
namespace RichTextEditor.Converters
{
    /// <summary> 
    ...
    internal static class XamlToHtmlConverter
    {
        ...
        /// </returns> 
        internal static string ConvertXamlToHtml(string xamlString, bool isMail)
        {
            //XmlTextReader xamlReader;
            //StringBuilder htmlStringBuilder;
            //XmlTextWriter htmlWriter;

            XmlTextReader xamlReader = new XmlTextReader(new StringReader(xamlString));

            StringBuilder htmlStringBuilder = new StringBuilder(100);
            XmlTextWriter htmlWriter = new XmlTextWriter(new StringWriter(htmlStringBuilder));

            if (!WriteFlowDocument(xamlReader, htmlWriter, isMail))
                return "";
        ...
        /// </param> 
        private static bool WriteFlowDocument(XmlTextReader xamlReader, XmlTextWriter htmlWriter, bool isMail)
        {
            ...
            // add mail header info:
            if (isMail)
            {
                htmlWriter.WriteStartElement("head");
                htmlWriter.WriteRaw("<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\">");
                htmlWriter.WriteRaw("<meta content=\"text/html; charset=utf-8\">");
